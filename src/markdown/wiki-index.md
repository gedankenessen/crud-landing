---
slug: "wiki"
type: "wiki-index"
title: "Wiki"
description: ""
---

## [What is crud?](/wiki/what)
- [Why crud?](/wiki/what#why)
- [Purpose](/wiki/what#purpose)
- [Limits](/wiki/what#limits)

## [Installation](/wiki/installation)
- [About](/wiki/installation#about)
- [Compose](/wiki/installation#docker-compose)
- [Docker](/wiki/installation#docker)
- [Git](/wiki/installation#git)
- [Customizing](/wiki/installation#customizing)
- [Whats next](/wiki/installation#next)

## [First steps](/wiki/first-steps)
- [The basics](/wiki/first-steps#basics)
- [Obtaining a token](/wiki/first-steps#token)
- [Creating your first endpoint](/wiki/first-steps#creating)
- [Querying data](/wiki/first-steps#querying)
- [Changing structure](/wiki/first-steps#changing)
- [Next steps](/wiki/first-steps#next)

## [API](/wiki/api)
- [Overview](/wiki/api#overview)
- [/build](/wiki/api#build)
- [/meta](/wiki/api#meta)
- [/user](/wiki/api#user)

## [Edgecases](/wiki/edgecases)
- [PUT does not trigger a clean-up](/wiki/edgecases#put)
- [`null` is treated as wildcard](/wiki/edgecases#null)
- [Unknown endpoints always return `[]`](/wiki/edgecases#unknown)

## [Architecture](/wiki/architecture)
- [Stack](/wiki/architecture#stack)
- [Deployment](/wiki/architecture#deployment)
- [Contributing](/wiki/architecture#contributing)
- [Namespaces](/wiki/architecture#namepsaces)
- [Data and errors](/wiki/architecture#data-and-errors)

